...
adjusted_data AS (
        select
                CONVERT(VARCHAR(100), [TYPE]) AS [TYPE],
                CONVERT(INTEGER, CODE) AS CODE,
                CONVERT(VARCHAR(100), DEBIT_CREDIT) AS DEBIT_CREDIT,
                CONVERT(VARCHAR(100),AFFECT ) AS AFFECT,
                CONVERT(VARCHAR(1024),[DESCRIPTION]) AS [DESCRIPTION],
                CONVERT(VARCHAR(65) ,TRANSACTION_TYPE) AS TRANSACTION_TYPE,
                CONVERT(VARCHAR(36) ,ACCOUNT_TYPE) AS ACCOUNT_TYPE,
                CONVERT(VARCHAR(1024) ,DESCRIPTION_CODE) AS DESCRIPTION_CODE,
                CONVERT(VARCHAR(1024) ,TREATMENT_CODE) AS TREATMENT_CODE
        FROM
                landing_data
)
SELECT *, GETUTCDATE() AS LOADED_AT FROM adjusted_data;
