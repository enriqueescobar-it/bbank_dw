-- NAME: SILVER_PRODUCT_FIS_CD300_DAILY
-- CATEGORY: MODEL
-- MATURITY LEVEL: 0
-- LAYER: SILVER
-- FREQUENCY: DAILY
...
-- depends_on: {{ ref('silver_product_fis_cd300') }}
-- depends_on: {{ ref('bronze_fis_cd300') }}
...
{% if is_incremental() %}
    WITH input_data AS (
        SELECT
            ...
            ,DATE_OF_DATA
            ,YEARMONTH
        FROM {{ref('silver_product_fis_cd300')}}
    )
{% endif %}

SELECT *, GETUTCDATE() AS LOADED_AT FROM input_data